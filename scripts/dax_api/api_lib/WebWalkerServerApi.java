...
public class WebWalkerServerApi implements Loggable {
    ...
    public PathResult getPath(final Point3D start, final Point3D end, final PlayerDetails playerDetails) {
        final com.google.gson.JsonObject pathRequest = new com.google.gson.JsonObject();
    ...
    public PathResult getBankPath(final Point3D start, final RunescapeBank bank, final PlayerDetails playerDetails) {
        final com.google.gson.JsonObject pathRequest = new com.google.gson.JsonObject();
    ...
    private PathResult parseResult(final ServerResponse serverResponse) {
        if (!serverResponse.isSuccess()) {

            final JsonValue jsonValue = Json.parse(serverResponse.getContents());
            if (!jsonValue.isNull()) {
    ...
    private ServerResponse post(final com.google.gson.JsonObject jsonObject, final String endpoint) throws IOException {
        ...
        final URL myurl = new URL(endpoint);
        final HttpURLConnection connection = (isTestMode ? (HttpURLConnection) myurl.openConnection() : (HttpsURLConnection) myurl.openConnection());
        ...
        final String contents = IOHelper.readInputStream(connection.getInputStream());
